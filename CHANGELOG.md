...
- Unified closing now uses one `close_timeout` budget for local Close writes, peer response, automatic control writes, and best-effort shutdown. A silent peer ends with `ConnectionClosed` once; cleanup failure or expiry preserves an already accepted Close or the original idle/Pong timeout. Compio application sends are rejected after local Close, and timed-out owned I/O is never restarted by a later flush.
