...
- Built-in HTTP/1 WebSocket server handshakes now reject nonzero or conflicting `Content-Length` values and any `Transfer-Encoding` request header; absent and zero-valued lengths remain accepted. Checked HTTP/1 client request construction now rejects custom `Content-Length` and `Transfer-Encoding` headers.
