...
- Tokio Sink readiness now drains queued encoded output at `max_backpressure`, continuing partial drains before accepting another message. This is a soft queue threshold, not a message size limit: individual messages may exceed it, and zero drains any pending output.
