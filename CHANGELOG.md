...
- Shared compression contexts now reuse role-aware encoder pools instead of allocating four encoders per connection. Client contexts honor `client_max_window_bits`, decoders remain connection-local, and a failed shared encoder is reset before reuse.
