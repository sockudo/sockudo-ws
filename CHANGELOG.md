...
- Reuse the compressed-input workspace when decoding subsequent messages that fit its capacity, avoiding one input allocation per message. Each decoder retains the workspace's peak capacity until it is dropped, including across context resets; input copying and output allocation remain.
