...
///
/// Reuses an input workspace across messages. Its capacity is retained until
/// the decoder is dropped, including after resetting the compression context.
...
impl DeflateDecoder {
    ...
    pub fn decompress(&mut self, data: &[u8], max_size: usize) -> Result<Bytes> {
        ...
        // Messages are decoded serially, so retain the workspace for reuse.
        self.input.clear();
        self.input.reserve(data.len() + DEFLATE_TRAILER.len());
